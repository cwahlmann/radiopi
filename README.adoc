...
[source, bash]
----
pi@raspberrypi:~/RadioPi sudo python3 calibrate_touch.py
----
After clicking with a touch pen on both crosses the file 'calibration.ini' will be generated.
If there is already one in the folder, a backup is made: '~calibration.ini'.

*Be careful: starting the script twice will overwrite the first backup.*
